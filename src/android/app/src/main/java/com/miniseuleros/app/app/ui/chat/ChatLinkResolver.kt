 ...
 *
 * Routing order:
 *  1. Recognized miniseuleros:// deep-link action  → DeepLink (delegated to MainActivity via Intent.ACTION_VIEW)
 *  2. miniseuleros://<sandbox path>, file://, or absolute /var/minis-euleros|/root path → SandboxFile
...
object ChatLinkResolver {
    ...
    fun resolve(rawUrl: String, sessionId: String? = null, context: Context? = null): ChatLinkAction {
        ...
        // 1. miniseuleros:// deep links — only branch out when the URL maps to a known action,
        //    otherwise fall through to sandbox-path handling.
        if (scheme == "miniseuleros") {
        ...
        if (com.miniseuleros.app.ui.browser.BrowserExternalSchemeHandler.shouldHandleExternally(trimmed)) {
     ...
     * Accepts:
     *   miniseuleros://attachments/foo.png        → /var/minis-euleros/attachments/foo.png
     *   miniseuleros:///var/minis-euleros/workspace/x.csv → /var/minis-euleros/workspace/x.csv (absolute)
     *   file:///path/to/file               → /path/to/file
     *   /var/minis-euleros/workspace/x.csv         → resolved via bind mount
    ...
    private fun resolveSandboxFile(
        ...
        return when (scheme) {
            "miniseuleros" -> {
                ...
                // `miniseuleros://` URLs don't use fragments, so stripping at '#'
                // would truncate filenames like `foo #China.mp4`.
                val stripped = raw.removePrefix("miniseuleros://").substringBefore('?')
                ...
                minisPathCandidates(stripped)
                    .asSequence()
                    .map { candidate ->
                        val linuxPath = if (candidate.startsWith("/")) candidate else "/var/minis-euleros/$candidate"
                        lookup(linuxPath)
                    }
                    ...
                    ?: lookup(
                        minisPathCandidates(stripped).first().let {
                            if (it.startsWith("/")) it else "/var/minis-euleros/$it"
                        },
                    )
            }
            "file" -> {
     ...
     * of a `miniseuleros://` URL, in priority order. Ported from iOS
     ...
     * and a web preview of a `miniseuleros://` URL renders nothing at all. There is
