package com.miniseuleros.app.provider.openai
...
import com.miniseuleros.app.data.model.LLMMediaAttachment
import com.miniseuleros.app.data.model.LLMMessage
import com.miniseuleros.app.data.model.LLMModel
import com.miniseuleros.app.data.model.LLMResponse
...
import com.miniseuleros.app.provider.thinking.ThinkingResolveContext
import com.miniseuleros.app.provider.thinking.ThinkingRuleResolver
import com.miniseuleros.app.provider.LLMProvider
import com.miniseuleros.app.provider.applyUserAgentOverride
...
import com.miniseuleros.app.provider.failOnSilentEmptyCompletion
...
class OpenAIProvider private constructor(
     ...
     * user-authored custom rules from [com.miniseuleros.app.provider.thinking.ThinkingRuleResolver]'s
     ...
     * the app-level [com.miniseuleros.app.data.FastModePrefs] toggle that Codex
    ...
    internal fun resolvedServiceTier(): String? =
        if (supportsPriorityProcessing && com.miniseuleros.app.data.FastModePrefs.isEnabled()) {
            "priority"
        } else {
            null
        }
    ...
    private val client = OkHttpClient.Builder()
        ...
        .connectionPool(com.miniseuleros.app.network.NetworkMonitor.sharedLLMConnectionPool)
    ...
    private fun rawStreamMessage(
        ...
        val memBefore = com.miniseuleros.app.diagnostics.MemorySnapshot.capture()
        val serStartNs = System.nanoTime()
        val bodyStr = try {
            body.toString()
        } catch (t: Throwable) {
            com.miniseuleros.app.diagnostics.LargeAllocProbe.report(
                "openai.body.toString", -1, "model=${model.id} messages=${messages.size}",
                memBefore, serStartNs, failure = t,
            )
            throw t
        }
        if (bodyStr.length >= com.miniseuleros.app.diagnostics.LargeAllocProbe.NOTABLE_BYTES) {
            com.miniseuleros.app.diagnostics.LargeAllocProbe.report(
        ...
        if (!response.isSuccessful) {
            ...
            com.miniseuleros.app.logging.AppLogger.error(
                "OpenAIProvider",
            ...
            if (com.miniseuleros.app.BuildConfig.DEBUG) {
                com.miniseuleros.app.debug.LLMRequestLog.add(
                    com.miniseuleros.app.debug.LLMRequestLog.Entry(
                        provider = "openai",
                ...
                run {
                    ...
                    if (delta != null) {
                        ...
                        if (cLen + rcLen + rLen + tcLen > 0 || delta.has("role")) {
                            com.miniseuleros.app.logging.AppLogger.debug(
                                "OpenAIProvider",
                    ...
                    msg.role == LLMMessage.Role.USER -> {
                        ...
                        for (tr in toolResults) {
                            ...
                            if (trBytes != null && trBytes.isNotEmpty() && supportsImages) {
                                val safeBytes = com.miniseuleros.app.provider.ImageBudget
                                    ...
                                    when (part) {
                                        ...
                                        is AgentContentPart.ImageData -> {
                                            if (supportsImages) {
                                                ...
                                                val safeBytes = com.miniseuleros.app.provider.ImageBudget.compressUnderBudget(part.data)
    ...
    private fun responsesImageBlock(
        ...
        // T-imgsize: provider-boundary backstop.
        val safeBytes = com.miniseuleros.app.provider.ImageBudget.compressUnderBudget(data)
    ...
    internal fun buildResponsesAPIBody(
        ...
        if (!body.has("service_tier") &&
            com.miniseuleros.app.data.FastModePrefs.isEnabled() &&
                        ...
                        if (imgParts.isNotEmpty()) {
                            ...
                            for (part in msg.contentParts) {
                                when (part) {
                                    ...
                                    else -> com.miniseuleros.app.logging.AppLogger.error(
                                        "OpenAIProvider",
