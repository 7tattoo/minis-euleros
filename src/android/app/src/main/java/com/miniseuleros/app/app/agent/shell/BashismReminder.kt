 ...
 * bashism rules but ran under POSIX sh (T-bash-on-demand §4.2). Isomorphic to
...
object BashismReminder {
    ...
    fun build(hits: List<BashismDetector.Hit>, installFailure: String?): String? {
        ...
        if (installFailure != null) {
            sb.appendLine("This command was executed by POSIX sh (NOT bash), because bash installation failed: ${sanitize(installFailure)}.")
        } else {
            sb.appendLine("This command was executed by POSIX sh (NOT bash).")
        }
        sb.appendLine("The script contains bash-only syntax that POSIX sh handles incorrectly, which is")
