package com.miniseuleros.app.provider.thinking
...
object ThinkingRuleResolver {
    ...
    fun anthropicThinkingShape(
        ...
        val adaptive = com.miniseuleros.app.provider.anthropic.AnthropicProvider
            .modelUsesAdaptiveThinking(modelId)
        if (level.isEnabled && supportsReasoning != false) {
            if (adaptive) {
                return mapOf(
                    "effort" to com.miniseuleros.app.provider.anthropic.AnthropicProvider
                        .thinkingEffort(level),
                )
            }
            val budget = com.miniseuleros.app.provider.anthropic.AnthropicProvider
