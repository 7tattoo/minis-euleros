...
object MountedFolderCoordinator {
    ...
    private const val MOUNTS_PREFIX = "/var/minis-euleros/mounts/"

    /**
     * Throws [ReadOnlyMountException] when [linuxPath] is inside an
     * effectively read-only mount. Paths outside `/var/minis-euleros/mounts/` and
    ...
    /**
     * True when [linuxPath] is `/var/minis-euleros/mounts/<name>/...` AND the
    ...
    fun isLinuxPathUnderReadOnlyMount(linuxPath: String, store: MountedFoldersStore): Boolean {
        ...
        // `/var/minis-euleros/mounts/foobar` as inside a mount named `foo`.
