package com.miniseuleros.app.webapp

import android.content.Context
import com.miniseuleros.app.data.db.WebAppShortcutEntity
 ...
 *      under /var/minis-euleros/<sub>/ are also accepted via resolveSessionHostPath)
...
object WebAppPathResolver {
     ...
     *
     * Mapping rules:
     *  - `/var/minis-euleros/shared` bind  → `pathScope = "shared"`,  `scopeContext = null`
     *  - `/var/minis-euleros/mounts/<n>`   → `pathScope = "mount"`,   `scopeContext = "<n>"`
    ...
    fun inferScope(hostFile: File): Triple<String, String?, String>? {
        ...
        val sorted = com.miniseuleros.app.sandbox.PRootKernel
        ...
        for ((linuxPrefix, hostBase) in sorted) {
            ...
            if (hostAbs == baseNorm || hostAbs.startsWith("$baseNorm/")) {
                ...
                return when {
                    linuxPrefix == "/var/minis-euleros/shared" ->
                        Triple(WebAppShortcutRepository.SCOPE_SHARED, null, linuxPath)
                    linuxPrefix.startsWith("/var/minis-euleros/mounts/") -> {
                        val mountName = linuxPrefix.removePrefix("/var/minis-euleros/mounts/")
                            .substringBefore('/')
    ...
    private fun resolveSession(context: Context, shortcut: WebAppShortcutEntity): File? {
        ...
        // Absolute /var/minis-euleros/<perSession>/... — go through PRootKernel which
        // knows how to map per-session subdirs to <filesDir>/minis-sessions/<id>/<sub>.
        if (shortcut.htmlPath.startsWith("/var/minis-euleros/")) {
