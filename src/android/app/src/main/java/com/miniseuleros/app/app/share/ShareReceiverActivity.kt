package com.miniseuleros.app.share
...
class ShareReceiverActivity : ComponentActivity() {
    ...
    private fun promptProviderImportOrAttach(
        ...
        return try {
            android.app.AlertDialog.Builder(this)
                .setTitle(getString(com.miniseuleros.app.R.string.share_provider_json_title))
                .setMessage(getString(com.miniseuleros.app.R.string.share_provider_json_message))
                .setPositiveButton(getString(com.miniseuleros.app.R.string.share_provider_json_import)) { _, _ ->
                ...
                .setNegativeButton(getString(com.miniseuleros.app.R.string.share_provider_json_attach)) { _, _ ->
                    finishWithAttachmentFlow(items)
                }
                .setOnCancelListener {
    ...
    private fun importProviderJson(json: String) {
        ...
        val repo = (applicationContext as? com.miniseuleros.app.MinisApp)?.providerRepositoryOrNull
        if (repo == null) {
            AppLogger.warning(TAG, "providerRepository unavailable; cannot import")
            toast(getString(com.miniseuleros.app.R.string.share_provider_json_import_failed))
            return
        }
        val label = repo.importInstanceJSON(json)
        if (label != null) {
            AppLogger.info(TAG, "imported provider \"$label\" from shared JSON")
            toast(getString(com.miniseuleros.app.R.string.share_provider_json_imported, label))
        } else {
    ...
    private fun launchMainActivity() {
        ...
        when (outcome) {
            ...
            ShareHandoffPolicy.Outcome.FAILED -> {
                ...
                try {
                    toast(getString(com.miniseuleros.app.R.string.share_open_app_failed))
                } catch (t: Throwable) {
    ...
    private fun mainActivityIntent(flags: Int): Intent =
        Intent(this, Class.forName("com.miniseuleros.app.MainActivity")).apply {
            addFlags(flags)
