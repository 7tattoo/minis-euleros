...
object MarkdownParser {
    ...
    fun parse(markdown: String): List<Block> {
        ...
        while (i < lines.size) {
            ...
            //   ![alt](miniseuleros://workspace/clip.mp4)
    ...
    private fun splitTableRow(line: String): List<String> {
        ...
        // contains a raw ASCII pipe, e.g. `[f](miniseuleros://ns/a|b.png)` — the model
        // (or a hand-authored row) can emit an un-encoded `|` inside the URL,
        // and a naive `split('|')` would chop the link in two. App-generated
        // miniseuleros:// URLs already percent-encode `|` to `%7C`, so this is purely
