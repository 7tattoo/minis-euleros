 ...
 * inside the sandbox rootfs but some dynamically linked binaries died with
 * SIGBUS / SIGSEGV. We could not reproduce it: on a Pixel 6 the very
 * same bytes (rootfs binaries verified byte-identical to the shipped tarball)
 * with the very same argv run to completion. So the fault is not
 ...
 * program** in the minirootfs — `/bin/true` and `uname` are symlinks into coreutils.
...
object SeccompFallbackPolicy {
     ...
     * dynamic link of a binary pulling in libcrypto/libssl on a slow device is
     ...
     *
     * - 135 = 128+7  SIGBUS  — reported for some dynamically linked binaries
     * - 139 = 128+11 SIGSEGV — reported for some dynamically linked binaries
