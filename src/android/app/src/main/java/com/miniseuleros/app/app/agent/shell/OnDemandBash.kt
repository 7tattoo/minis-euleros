package com.miniseuleros.app.agent.shell
 ...
 *   - host-side 5s network precheck before running yum (F2),
 *   - persistent failure backoff (24h / 3-strikes) via SharedPreferences (F2),
 *   - one install attempt per process launch.
 *
 * The bundled EulerOS minirootfs ships bash by default, so this path normally
 * short-circuits at the `command -v bash` probe. The yum fallback is kept for
 * the rare case where bash was removed from the rootfs.
 */
object OnDemandBash {
    ...
    private const val REPO_PROBE_URL = "https://repo.huaweicloud.com/hce/2.0/os/aarch64/repodata/repomd.xml"
    ...
    suspend fun ensureBash(context: Context, executor: Executor): Outcome = lock.withLock {
        ...
        if (!networkReachable()) {
            ...
            return Outcome.Unavailable("network/repo mirror unreachable")
        }

        Log.i(TAG, "installing bash (budget ${INSTALL_BUDGET_MS / 1000}s)…")
        val rc = executor.run("yum install -y bash", INSTALL_BUDGET_MS)
        ...
        if (verified) {
            ...
            // if bash is later removed from the rootfs this session, one fresh reinstall
        ...
        return Outcome.Unavailable("yum install bash failed (rc=$rc)")
    }

    /** Called when `bash <file>` itself returned 127 (user removed bash) — M5. */
    ...
    private fun backoffReason(context: Context): String? {
        ...
        if (count >= MAX_STRIKES) return "bash install disabled after $MAX_STRIKES failures (retry manually: yum install -y bash)"
    ...
    private suspend fun networkReachable(): Boolean = withContext(Dispatchers.IO) {
        try {
            val c = (URL(REPO_PROBE_URL).openConnection() as HttpURLConnection).apply {
