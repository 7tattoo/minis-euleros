...
class BackupExporter(
    ...
    private suspend fun exportProviders(
        ...
        val config = repo?.config?.value ?: com.miniseuleros.app.data.model.ProviderConfig()
        val url = File(dataDir, "provider_config.json")
        url.writeText(
            BackupFormat.json.encodeToString(
                com.miniseuleros.app.data.model.ProviderConfig.serializer(), config
            )
        )
    ...
    private suspend fun exportThinkingRules(dataDir: File): Int {
        val dao = com.miniseuleros.app.data.db.ProviderDatabase
