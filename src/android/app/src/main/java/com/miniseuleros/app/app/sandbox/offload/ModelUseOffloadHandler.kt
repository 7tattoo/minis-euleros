package com.miniseuleros.app.sandbox.offload
...
import com.miniseuleros.app.provider.safeOptString
import com.miniseuleros.app.sandbox.NativeOffloadHandler
import com.miniseuleros.app.sandbox.NativeOffloadRequest
import com.miniseuleros.app.sandbox.NativeOffloadResult
...
/**
 * `minis-model-use` — list, search, and invoke LLM models from EulerOS shell.
...
class ModelUseOffloadHandler(
    ...
    private fun cmdRun(args: OffloadArgs, request: NativeOffloadRequest): NativeOffloadResult {
        ...
        if (hasAudioInput && provider !is com.miniseuleros.app.provider.openai.OpenAIProvider) {
            return NativeOffloadResult(
                2,
        ...
        if (ptSpec.active) {
            val openAI = provider as? com.miniseuleros.app.provider.openai.OpenAIProvider
                ?: return NativeOffloadResult(
                    2,
        ...
        (provider as? com.miniseuleros.app.provider.openai.OpenAIProvider)?.let { openAI ->
    ...
    private data class ImageGenConfig(
        ...
        val endpointOverride: com.miniseuleros.app.data.model.ImageEndpointMode? = null,
    )
    ...
    private fun parseImageGenConfig(inputJson: String): ImageGenConfig {
        ...
        val endpoint = when (rawEndpoint.lowercase()) {
            "auto" -> com.miniseuleros.app.data.model.ImageEndpointMode.auto
    ...
    private fun performRawPassthrough(
        ...
        entry: ModelEntry,
        openAI: com.miniseuleros.app.provider.openai.OpenAIProvider,
        systemPrompt: String?,
    ...
    private fun tryImageGenerationRoute(
        ...
        provider: com.miniseuleros.app.provider.LLMProvider,
        inputJson: String,
        ...
        if (instance.credentialType == com.miniseuleros.app.data.model.ProviderCredential.oauth) return null
        ...
        val effectiveMode = when {
            ...
            isPureImageGenerator -> com.miniseuleros.app.data.model.ImageEndpointMode.imagesGenerations
            instance.imageEndpointMode == com.miniseuleros.app.data.model.ImageEndpointMode.auto &&
        ...
        if (effectiveMode == com.miniseuleros.app.data.model.ImageEndpointMode.chatCompletions) return null
        ...
        if (effectiveMode == com.miniseuleros.app.data.model.ImageEndpointMode.imagesGenerations) {
            val response = try {
                runBlocking {
        ...
        val response = try {
            runBlocking {
                ...
                providerRepository.setImageEndpointResolved(
                    instance.id, com.miniseuleros.app.data.model.ImageEndpointMode.imagesGenerations,
                )
            }
        } catch (e: Throwable) {
            val msg = e.message ?: ""
            val routeMissing = (e is com.miniseuleros.app.data.model.LLMError.ProviderError ||
                e is com.miniseuleros.app.data.model.LLMError.InvalidApiKey) && looksLikeEndpointMissing(msg)
            if (routeMissing) {
                Log.i(TAG, "[ModelUseRoute] auto probe → route missing (${msg.take(120)}) — caching chat_completions, falling back")
                providerRepository.setImageEndpointResolved(
                    instance.id, com.miniseuleros.app.data.model.ImageEndpointMode.chatCompletions,
                )
    ...
    private fun writeImageResult(
        entry: ModelEntry,
        response: com.miniseuleros.app.data.model.LLMResponse,
        outputPath: String?,
        outputExt: String,
        endpointUsed: String,
        sessionId: String?,
    ...
    private fun entryDict(entry: ModelEntry): JSONObject =
        com.miniseuleros.app.offload.ModelUseManager.entryDict(
            entry,
