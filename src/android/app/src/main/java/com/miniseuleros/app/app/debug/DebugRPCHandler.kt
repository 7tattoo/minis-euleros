package com.miniseuleros.app.debug
...
class DebugRPCHandler(private val context: Context) {
    ...
    private fun crashSummary(f: File): String = try {
        ...
        val frame = body.firstOrNull { it.trimStart().startsWith("at com.miniseuleros") }
    ...
    private suspend fun handleShellExecute(params: JSONObject): JSONObject {
        ...
        val result = try {
            ExecutionCoordinator.execute(
                ...
                lineCallback = { rawLine ->
                    val (_, urls) = com.miniseuleros.app.terminal.MinisUrlMarker.extract(rawLine)
        ...
        for (raw in capturedUrls) {
            com.miniseuleros.app.terminal.MinisOpenUrlBroker.offer(raw)
        }
        return JSONObject()
    ...
    private suspend fun handleUpdateCheck(): JSONObject {
        return when (val r = com.miniseuleros.app.data.UpdateChecker.check()) {
            is com.miniseuleros.app.data.UpdateChecker.CheckResult.UpdateAvailable -> JSONObject()
            ...
            com.miniseuleros.app.data.UpdateChecker.CheckResult.UpToDate ->
                JSONObject().put("status", "up_to_date")
            com.miniseuleros.app.data.UpdateChecker.CheckResult.NoReleaseAvailable ->
                JSONObject().put("status", "no_release")
            is com.miniseuleros.app.data.UpdateChecker.CheckResult.NoApkAsset ->
                JSONObject().put("status", "no_apk_asset").put("tag_name", r.tagName)
            com.miniseuleros.app.data.UpdateChecker.CheckResult.Forbidden ->
                JSONObject().put("status", "forbidden")
            com.miniseuleros.app.data.UpdateChecker.CheckResult.NetworkUnreachable ->
                JSONObject().put("status", "network_unreachable")
            is com.miniseuleros.app.data.UpdateChecker.CheckResult.Error ->
    ...
    private suspend fun handleUpdateDownload(params: JSONObject): JSONObject {
        ...
        return when (val r = com.miniseuleros.app.data.UpdateChecker.download(context, url)) {
            is com.miniseuleros.app.data.UpdateChecker.DownloadResult.Success -> JSONObject()
                .put("status", "ok")
                .put("path", r.file.absolutePath)
                .put("size", r.file.length())
            is com.miniseuleros.app.data.UpdateChecker.DownloadResult.Error -> JSONObject()
                .put("status", "error")
    ...
    private fun handleUpdateInstall(params: JSONObject): JSONObject {
        ...
        val canInstall = com.miniseuleros.app.data.UpdateChecker.canInstall(context)
        return try {
            com.miniseuleros.app.data.UpdateChecker.installApk(context, file)
    ...
    private fun handleWriteFile(params: JSONObject): JSONObject {
        ...
        val hostFile = PRootKernel.resolveHostPath(path) ?: run {
            val rootfsDir = com.miniseuleros.app.sandbox.RootfsManager.getInstance(context).rootfsDir
    ...
    private fun handlePermissionsList(): JSONObject {
        ...
        for (tool in com.miniseuleros.app.offload.OffloadPermissionManager.toolRegistry) {
            arr.put(JSONObject()
                ...
                .put("currentLevel", com.miniseuleros.app.offload.OffloadPermissionManager.getLevel(tool.toolName).name)
    ...
    /**
     * T344: Direct invocation of [com.miniseuleros.app.sandbox.offload.ShizukuOffloadHandler]
    ...
    private fun handleShizukuExec(params: JSONObject): JSONObject {
        ...
        val handler = com.miniseuleros.app.sandbox.offload.ShizukuOffloadHandler(context)
        ...
        val request = com.miniseuleros.app.sandbox.NativeOffloadRequest(
            pid = -1,
    ...
    /**
     * Direct invocation of [com.miniseuleros.app.sandbox.offload.ModelUseOffloadHandler]
     * for e2e harnesses. Mirrors [handleShizukuExec]; lets callers exercise the
     * `minis-model-use` CLI without going through a real EulerOS shell prompt.
     * DEBUG-only.
     */
    private fun handleModelUseExec(params: JSONObject): JSONObject {
        ...
        val finalArgv: List<String> = if (params.has("input")) {
            ...
            val hostFile = com.miniseuleros.app.sandbox.PRootKernel.resolveHostPath(linuxPath)
        ...
        val app = context.applicationContext as com.miniseuleros.app.MinisApp
        val handler = com.miniseuleros.app.sandbox.offload.ModelUseOffloadHandler(context, app.providerRepository)
        val request = com.miniseuleros.app.sandbox.NativeOffloadRequest(
            pid = -1,
     ...
     * [com.miniseuleros.app.sandbox.offload.SessionsOffloadHandler] for e2e
     * harnesses. Mirrors [handleModelUseExec]; lets callers exercise the
     * `minis-sessions-cli` CLI (list / search / messages, incl. --full)
     * without going through a real EulerOS shell prompt. DEBUG-only.
     */
    private fun handleSessionsExec(params: JSONObject): JSONObject {
        ...
        val app = context.applicationContext as com.miniseuleros.app.MinisApp
        val handler = com.miniseuleros.app.sandbox.offload.SessionsOffloadHandler(app.chatRepository)
        val request = com.miniseuleros.app.sandbox.NativeOffloadRequest(
            pid = -1,
        ...
        return when (sub) {
            "set" -> {
                ...
                kotlinx.coroutines.runBlocking {
                    com.miniseuleros.app.config.ConfigBridge.performWriteBatch(
                        items = items,
            ...
            "get" -> {
                ...
                com.miniseuleros.app.config.ConfigBridge.readField(
                    path = path,
                    filter = filter,
                    page = page,
                    pageSize = pageSize,
            ...
            "topics" -> JSONObject().apply {
                put("ok", true)
                put("topics", com.miniseuleros.app.config.ConfigBridge.allTopics())
            }
            "topic-help" -> {
                ...
                val known = com.miniseuleros.app.config.ConfigBridge.allTopics()
                    .let { arr -> (0 until arr.length()).any { arr.optString(it) == topic } }
                if (!known) {
                    throw RPCException(-32602, "Unknown topic '$topic' — call subcommand=topics")
                }
                val fields = com.miniseuleros.app.config.ConfigBridge.fieldsForTopic(topic)
                JSONObject().apply {
                    put("ok", true)
                    put("topic", topic)
            ...
            "audit-list" -> {
                ...
                com.miniseuleros.app.config.ConfigBridge.auditList(limit, scope)
            }
