...
import com.miniseuleros.app.ui.components.DialogTextField
...
fun MCPIntegrationsScreen(
    ...
    envVarRepository: com.miniseuleros.app.data.repository.EnvVarRepository? = null,
) {
...
private fun MCPAddSheet(
    mcpRepository: MCPRepository,
    envVarRepository: com.miniseuleros.app.data.repository.EnvVarRepository?,
...
private fun MCPFormTab(
    ...
    val appEnvEntries by (envVarRepository?.entries
        ?.collectAsState() ?: remember { mutableStateOf(emptyList<com.miniseuleros.app.data.repository.EnvVarRepository.EnvVarEntry>()) })
    ...
    var oauthClientSecret by remember {
        mutableStateOf(
            editServer?.id?.let {
                com.miniseuleros.app.mcp.oauth.MCPOAuthStore.clientSecret(context, it)
            } ?: "",
        )
    }
    ...
    var oauthAuthorized by remember {
        mutableStateOf(
            editServer?.id?.let {
                com.miniseuleros.app.mcp.oauth.MCPOAuthStore.isAuthorized(context, it)
            } ?: false,
        )
    }
    var oauthBusy by remember { mutableStateOf(false) }

    fun currentOAuthConfig(): com.miniseuleros.app.mcp.oauth.MCPOAuthConfig? {
        val cfg = com.miniseuleros.app.mcp.oauth.MCPOAuthConfig(
                    ...
                    if (oauthAuthorized) {
                        MinisTextButton(
                            ...
                            onClick = {
                                val sid = name.trim()
                                if (sid.isNotEmpty()) {
                                    com.miniseuleros.app.mcp.oauth.MCPOAuthStore.signOut(context, sid)
                        ...
                        onClick = {
                            ...
                            when {
                                ...
                                cfg == null ->
                                    ...
                                    com.miniseuleros.app.mcp.oauth.MCPOAuthStore.setClientSecret(
                                        context, sid, oauthClientSecret.trim().ifBlank { null },
                                    )
                                    oauthBusy = true
                                    oauthScope.launch {
                                        val result = com.miniseuleros.app.mcp.oauth.MCPOAuthController(context)
                                            .authorize(sid, url.trim(), cfg)
                                        oauthBusy = false
                                        when (result) {
                                            is com.miniseuleros.app.mcp.oauth.MCPOAuthController.Result.Success -> {
                                            ...
                                            is com.miniseuleros.app.mcp.oauth.MCPOAuthController.Result.Cancelled -> {}
                                            is com.miniseuleros.app.mcp.oauth.MCPOAuthController.Result.Failed -> {
