...
class DebugServer(
     ...
     *   adb shell run-as com.miniseuleros.app cat files/debug_server_token
        ...
        socket.use { s ->
            try {
                ...
                if (!isAuthorized(isLoopback, providedToken, authToken)) {
                    ...
                    sendResponse(writer, 401, rpcHandler.errorJSON(-32000, "Unauthorized — send X-Minis-Token (see `adb shell run-as com.miniseuleros.app cat files/debug_server_token`)"))
                    return
                }
